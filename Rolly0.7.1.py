...
controller_font = pygame.font.Font(None, 24)  # Small font for controller indicators
...
def reset_balls(fallen_ball=None):
	...
	if selected_player_count <= 2 or selected_game_mode == "Last Standing":
	    if selected_game_mode != "Co-op": # Skip for Co-op mode
	        # Draw current state
...
def title_screen():
    ...
    while True:
                ...
                if event.key == K_RETURN:
                    ...
                    if selected_option == 0:  # Players
                        ...
                        if selected_player_count > 1:  # Allow mode change for 2+ players, including Co-op
                            ...
                            # Force normal mode if Last Standing with only 2 players
                            if selected_player_count == 2 and selected_game_mode == "Last Standing":
                                selected_game_mode = "Normal"
        ...
        # Only grey out Last Standing for 2 players, Co-op is always available for 2+ players
        mode_color = GRAY if (selected_player_count == 2 and selected_game_mode == "Last Standing") else color
	...
	if selected_game_mode == "Co-op":
        ...
        if len(controllers) > 0:
            ...
            draw_text_with_outline(controller_text, subtitle_font, WIDTH // 2, HEIGHT * 4 // 5, 
...
def move_ball(ball, keys, up, down, left, right, controller_index=-1):
    ...
    if controller_index >= 0 and controller_index < len(controllers):
        ...
        ball['vx'] += x_axis * BALL_SPEED * 1.5  # Slightly higher sensitivity for controller
...
def draw_controller_indicators():
    ...
    if len(controllers) > 3 and 'purple' in active_balls:
        controller_text = "🎮4"
        draw_text_with_outline(controller_text, controller_font, 
                             WIDTH - 60, HEIGHT - 15, PLAYER_PURPLE, BLACK, center=True)
    ...
    while waiting:
        ...
        if current_time - start_time > 1000:  # Prevent immediate skip
            for i in range(len(controllers)):
                if controllers[i].get_button(0) or controllers[i].get_button(7):  # A or Start button
                    waiting = False
                    break
        ...
        if current_time - start_time > 500:  # Slight delay before showing text
            ...
            if current_time - start_time > 1000:
                draw_text_with_outline("Press Enter or Controller Button to Play Again", 
...
def move_ai_ball_coop(ai_ball, player_ball, current_time):
    ...
    if player_has_powerup and dist_to_player < 300:
        ...
        if player_ball['powerup'] in ['heavy', 'grow', 'tron']:
            ...
            if dist_to_player > 0:
                move_x -= (dx / dist_to_player) * BALL_SPEED * AI_SPEED * 1.5 * speed_mult
                move_y -= (dy / dist_to_player) * BALL_SPEED * AI_SPEED * 1.5 * speed_mult
        else:
            ...
            if closest_powerup:
                ...
                angle = math.atan2(dy, dx) + math.pi/2  # 90 degrees offset for circling
...
def apply_powerup(ball, powerup_type):
    ...
    if powerup_type == 'tron':
        ...
        if selected_game_mode == "Co-op":
            ...
            for ai_ball in coop_ai_balls:
                ai_ball['frozen'] = True
                ai_ball['frozen_end'] = current_time + POWERUP_DURATIONS['freeze']
...
def spawn_coop_ai_balls():
    ...
    for i in range(coop_level):
        ...
        x = max(PLATFORM_X + BALL_RADIUS, min(x, PLATFORM_X + PLATFORM_SIZE - BALL_RADIUS))
        y = max(PLATFORM_Y + BALL_RADIUS, min(y, PLATFORM_Y + PLATFORM_SIZE - BALL_RADIUS))
        ...
        ai_ball = {
            ...
            'tron_trail': [],
            'is_ai': True,
            'speed_multiplier': 1.0 + (0.1 * min(coop_level - 1, 5))  # Speed increases with level, caps at +50%
        }
...
def show_level_message():
    ...
    global coop_leel
    screen.fill(BACKGROUND_COLOR)
    draw_text_with_outline(f"Level {coop_level}", title_font, WIDTH // 2, HEIGHT // 3, WHITE, BLACK, center=True)
    ...
    if active_players:
        draw_text_with_outline("Active Players:", subtitle_font, WIDTH // 2, y_pos, WHITE, BLACK, center=True)
        y_pos += 40
        
        for color in active_players:
            draw_text_with_outline(color.capitalize(), subtitle_font, WIDTH // 2, y_pos, 
                                player_colors[color], BLACK, center=True)
            y_pos += 30
    
    # Show enemy count
    draw_text_with_outline(f"{coop_level} AI Opponent{'' if coop_level == 1 else 's'}", 
                         subtitle_font, WIDTH // 2, HEIGHT * 3 // 4 - 30, AI_GRAY, BLACK, center=True)
    
    # Ready text
    draw_text_with_outline("Get Ready!", subtitle_font, WIDTH // 2, HEIGHT * 3 // 4 + 30, WHITE, BLACK, center=True)
...
def show_game_over_screen():
    ...
    while waiting:
        ...
        if current_time - start_time > 1000:
            for i in range(len(controllers)):
                if controllers[i].get_button(0) or controllers[i].get_button(7):
                    waiting = False
                    break
        ...
        # Draw game over text
        draw_text_with_outline("Game Over!", title_font, WIDTH // 2, HEIGHT // 4, WHITE, BLACK, center=True)
        
        # Draw level reached
        level_text = f"Your team reached Level {coop_level}"
        draw_text_with_outline(level_text, subtitle_font, WIDTH // 2, HEIGHT // 2 - 30, WHITE, BLACK, center=True)
        
        # Draw AI defeated count
        ai_count = coop_level - 1  # Players defeated all AI in previous levels
        ai_text = f"AI Defeated: {ai_count}"
        draw_text_with_outline(ai_text, subtitle_font, WIDTH // 2, HEIGHT // 2 + 30, AI_GRAY, BLACK, center=True)
        
        # Show continue prompt
        if current_time - start_time > 1000:
            draw_text_with_outline("Press Enter or Controller Button to Continue", 
                                 subtitle_font, WIDTH // 2, HEIGHT * 3 // 4, 
                                 WHITE, BLACK, center=True)
...
def show_victory_screen():
    ...
    while waiting:
        ...
        # Draw victory text
        draw_text_with_outline("VICTORY!", title_font, WIDTH // 2, HEIGHT // 3, WHITE, BLACK, center=True)
        
        # Draw congratulatory text
        victory_text = f"You beat all {MAX_AI_BALLS} levels!"
        draw_text_with_outline(victory_text, subtitle_font, WIDTH // 2, HEIGHT // 2, WHITE, BLACK, center=True)
...
def coop_game():
    ...
    while True:
        ...
        if 'red' in active_players:
            move_ball(red_ball, keys, pygame.K_w, pygame.K_s, pygame.K_a, pygame.K_d, 0)
        ...
        for ai_ball in coop_ai_balls:
            ...
            # Check frozen state
            if ai_ball.get('frozen', False) and current_time >= ai_ball.get('frozen_end', 0):
        ...
        draw_text_with_outline(f"Level {coop_level}", subtitle_font, WIDTH // 2, 20, WHITE, BLACK, center=True)
...
def draw_coop_status():
    ...
    # Only show status for players that should be in the game based on player count
    active_colors = ['red']
    if selected_player_count >= 2:
    ...
    for color in active_colors:
        ...
        draw_text_with_outline(f"{color.capitalize()}: {status}", score_font, x, y, text_color, BLACK, center=False)
